...
Canonical HTML: https://docs.fenic.ai/latest/topics/create_dataframe_schema/
...
- **No unknown columns.** The data may not contain columns outside the schema.
  Column *order* in the input does not matter — the result is ordered by the
  schema.
...
**No content validation.** An explicit schema tags a column's *logical type*; it
does not validate the *content*. A `JsonType` column is not parsed or checked for
...
[JSON Processing](https://docs.fenic.ai/latest/examples/json_processing/index.md) and
[Markdown Processing](https://docs.fenic.ai/latest/examples/markdown_processing/index.md) examples.
...
[Timestamps and Dates](https://docs.fenic.ai/latest/topics/timestamps_and_dates/index.md).
