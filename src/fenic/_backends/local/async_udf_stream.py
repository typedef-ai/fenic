...
class AsyncUDFSyncStream:
    ...
    - Guarantees:
        ...
        * External cancellation via cancel_pending_tasks()
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Make dictionaries and state members for external access
    ...
    async def _call_batch_async(self, items: Iterable[Dict[str, Any]]) -> AsyncGenerator[Any, None]:
        ...
        Uses class members for state so external cancellation is possible.
        """
        ...
        try:
            while self._pending or self._results:
                ...
                while self._next_to_yield in self._results:
                    yield self._results.pop(self._next_to_yield)
                ...
                tasks_to_wait = {self._pending[self._next_to_yield]} if len(self._results) >= self.max_buffer_size else set(self._pending.values())
