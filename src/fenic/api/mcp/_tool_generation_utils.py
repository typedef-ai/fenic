    ...
    *,
    ...
    if not table_names:
        raise ConfigurationError(
            "At least one table name must be specified for automated system tool creation."
        )
...
def _auto_generate_system_tools(
    ...
    *,
    ...
    group_desc = "\n".join(
        [
            f"{d.table_name}: {d.description.strip()}"
            if d.description
            else d.table_name
            for d in datasets
        ]
    ...
    generated_tools: List[SystemTool] = [
        ...
        _auto_generate_search_summary_tool(
            name_to_spec,
            session,
            tool_name=f"{tool_namespace} - Search Summary"
            if tool_namespace
            else "Search Summary",
            tool_description="\n".join(
                [
        ...
        _auto_generate_search_content_tool(
            name_to_spec,
            session,
            tool_name=f"{tool_namespace} - Search Content"
            if tool_namespace
            else "Search Content",
            tool_description="\n".join(
                [
...
def _build_datasets_from_tables(
    table_names: List[str], session: Session
) -> List[ToolDataset]:
...
def _auto_generate_read_tool(
    ...
    *,
    ...
    def read_func(
        ...
        limit: Annotated[
            Optional[int], "Max rows to read within a page"
        ] = result_limit,
        offset: Annotated[
            Optional[int], "Row offset to start from (requires order_by)"
        ] = None,
        order_by: Annotated[
            Optional[str],
            "Comma separated list of columns to order by (required for offset)",
        ] = None,
        sort_ascending: Annotated[
            Optional[bool], "Sort ascending for all order_by columns"
        ] = True,
        include_columns: Annotated[
            Optional[str], "Comma separated list of columns to include in the result"
        ] = None,
        exclude_columns: Annotated[
            Optional[str], "Comma separated list of columns to exclude from the result"
        ] = None,
    ) -> LogicalPlan:
        ...
        order_by = (
            [c.strip() for c in order_by.split(",") if c.strip()] if order_by else None
        )
        available_columns = df.columns
        include_columns = (
            [c.strip() for c in include_columns.split(",") if c.strip()]
            if include_columns
            else None
        )
        exclude_columns = (
        ...
        if include_columns and exclude_columns:
            raise ValidationError(
                "include_columns and exclude_columns cannot be used together."
            )
        if include_columns:
        ...
        if exclude_columns:
            filtered_columns = [
                c for c in available_columns if c not in exclude_columns
            ]
            df = df.select(*filtered_columns)
...
def _auto_generate_search_summary_tool(
    ...
    def search_summary(
        ...
        for name, dataset in datasets.items():
            ...
            cols = [
                f.name for f in df.schema.column_fields if f.data_type == StringType
            ]
            if not cols:
...
def _auto_generate_search_content_tool(
    ...
    *,
    ...
    if len(datasets) == 0:
        raise ValidationError(
            "Cannot create search content tool: no datasets provided."
        )
    ...
    def _string_columns(df: DataFrame, selected: Optional[List[str]]) -> List[str]:
        if selected:
            ...
            if missing:
                raise ValidationError(
                    f"Column(s) {missing} not found. Available: {', '.join(df.columns)}"
                )
            return selected
    ...
    def search_rows(
        ...
        limit: Annotated[
            Optional[int], "Max rows to read within a page of search results"
        ] = result_limit,
        offset: Annotated[
            Optional[int], "Row offset to start from (requires order_by)"
        ] = None,
        order_by: Annotated[
            Optional[str],
            "Comma separated list of column names to order by (required with offset)",
        ] = None,
...
def _auto_generate_schema_tool(
    ...
    def schema_func(
        df_name: Annotated[
            str | None,
            "Optional DataFrame name to return a single schema for. To return schemas for all datasets, OMIT this parameter.",
        ] = None,
    ) -> LogicalPlan:
        ...
        for name, dataset in selected.items():
            ...
            schema_entries = [
                ...
                for f in dataset.schema(session).column_fields
            ]
...
def _auto_generate_sql_tool(
    ...
    *,
    ...
    def analyze_func(
        full_sql: Annotated[
            str,
            "Full SELECT SQL. Refer to DataFrames by name in braces, e.g., `SELECT * FROM {orders}`. JOINs between the provided datasets are allowed. SQL dialect: DuckDB. DDL/DML and multiple top-level queries are not allowed",
        ],
    ) -> LogicalPlan:
        return session.sql(
            full_sql.strip(),
            **{spec.table_name: spec.df(session) for spec in datasets.values()},
        )._logical_plan
...
def _apply_paging(
    ...
    *,
    ...
    if offset is not None and not order_by:
        raise ValidationError(
            "offset requires order_by to ensure deterministic paging."
        )
...
class ProfileRow:
    ...
    semantic_type: Literal[
        "identifier", "categorical", "continuous", "text", "boolean", "unknown"
    ]
...
def _auto_generate_profile_tool(
    ...
    *,
    ...
    def profile_func(
        df_name: Annotated[
            str | None,
            "Optional DataFrame name to return a single profile for. To return profiles for all datasets, omit this parameter.",
        ] = None,
    ) -> LogicalPlan:
        # sometimes the models get...very confused, and pass the null string instead of `null` or omitting the field entirely
        if not df_name or df_name == "null":
...
def _compute_profile_for_dataset(
    ...
    top_values_struct = pl.Struct(
        [pl.Field("value", pl.Utf8), pl.Field("count", pl.Int64)]
    )
...
def _compute_profile_rows(
    ...
    for field in df.schema.column_fields:
        ...
        boolean_stats = BooleanStats(
            true_rows=agg_row.get(a_true(col_name))
            if agg_row.get(a_true(col_name)) is not None
            else None,
            false_rows=(sample_size - agg_row.get(a_true(col_name)))
            if agg_row.get(a_true(col_name)) is not None
            else None,
        )
        stats = ProfileRow(
        ...
        if field.data_type in numeric_types:
            ...
            if stats.string_stats.distinct_count is not None:
                distinct_ratio = (
                    (stats.string_stats.distinct_count / sample_size)
                    if sample_size > 0
                    else 0
                )
                if distinct_ratio > 0.95:
                    ...
                    stats.semantic_type = (
                        "identifier"
                        if (
                            stats.string_stats.avg_length_chars is not None
                            and stats.string_stats.avg_length_chars < 50
                        )
                        else "text"
                    )
            ...
            if (
                stats.string_stats.avg_length_chars is not None
                and stats.string_stats.avg_length_chars
            ...
            compute_topk = (
                ...
                stats.string_stats.distinct_count is not None
                and stats.string_stats.distinct_count <= max(topk_distinct * 10, 200)
            )
            ...
            if compute_topk:
                ...
                if stats.percent_rows_contains_null < 100:
                    ...
                    sampled = s_non_null.sample(
                        n=k, with_replacement=False, shuffle=True
                    ).to_list()
                    ...
                    stats.string_stats.example_values = [
                        (
                            v[:LONG_TEXT_COLUMN_THRESHOLD_CHAR_LENGTH]
                            + f"... (truncated {len(v) - LONG_TEXT_COLUMN_THRESHOLD_CHAR_LENGTH} characters)"
                        )
                        if isinstance(v, str)
                        and len(v) > LONG_TEXT_COLUMN_THRESHOLD_CHAR_LENGTH
                        else v
                        for v in sampled
                    ]
