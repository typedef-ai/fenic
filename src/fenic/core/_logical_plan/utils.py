...
def validate_model_support_for_pdf_parsing(model_alias: Optional[ResolvedModelAlias], resolved_session_config: ResolvedSessionConfig):
    ...
    if not completion_parameters.supports_pdf_parsing:
        raise ValidationError(f"fenic does not support parsing with [{model_provider.value}:{model_config.model_name}]. Supported models: {model_catalog.get_models_supporting_pdf_parsing_as_string()}")
