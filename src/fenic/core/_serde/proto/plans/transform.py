...
def _deserialize_semantic_cluster(
    ...
    input_plan = context.deserialize_logical_plan(
        SerdeContext.INPUT, semantic_cluster.input, session_state=session_state
    )
    by_expr = context.deserialize_logical_expr(
        "by_expr", semantic_cluster.by_expr
    )
    centroid_column = (
        semantic_cluster.centroid_column if semantic_cluster.centroid_column else None
    )
    result = SemanticCluster(
        input=input_plan,
        by_expr=by_expr,
        num_clusters=semantic_cluster.num_clusters,
        label_column=semantic_cluster.label_column,
