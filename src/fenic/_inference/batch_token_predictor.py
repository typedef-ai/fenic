...
class CompletionsBatchTokenPredictor(BatchTokenPredictor[FenicCompletionsRequest, FenicCompletionsResponse]):
    ...
    def _sample_statistics(self, sample_tokens: list[int]) -> tuple[int, float]:
        ...
        print(f"Sample statistics: median={sample_median:.2f}, mean={sample_mean:.2f}, stddev={sample_stddev:.2f}, max={sample_max:.2f}, min={sample_min:.2f}, distribution_shape={distribution_shape}")
    ...
    def _predict_skewed(self, tokens: np.ndarray, distribution_shape: DistributionShape, median: float) -> tuple[int, float]:
