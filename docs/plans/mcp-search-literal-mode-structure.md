...
- **[applied]** Repo validation recipes were derived from `CLAUDE.md`, `justfile`, and `pyproject.toml`. `just` is available after `mise use just`, which records the tool in `mise.toml`; this shell resolved it to `just 1.54.0`. The direct `uv run ...` commands remain valid targeted equivalents for the per-file checks.
...
> Implement the phases in order. Run `mise install` if needed so the `just` tool from `mise.toml` is on PATH; if `just` is still unavailable, run `mise use just` from the repo root and then use the repo `just` recipes normally. After editing any fenic pipeline examples, run `fenic check <file>`; this plan does not currently require editing fenic pipeline examples.
