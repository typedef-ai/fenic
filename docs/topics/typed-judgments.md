...
Inference SDK clients use a 600-second transport ceiling. Startup validation uses
a separate 10-second client, while a shorter `request_timeout` remains enforced
by the scheduler for each request.
