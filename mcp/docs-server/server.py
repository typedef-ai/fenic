...
# Change to writable directory for DuckDB files
    ...
    if not session:
        try:
            ...
            if not os.environ.get("OPENAI_API_KEY") and not os.environ.get("GEMINI_API_KEY"):
                import sys
                print("Warning: No API keys found. Set OPENAI_API_KEY and/or GEMINI_API_KEY environment variables.", file=sys.stderr)
            ...
            import sys
            print(f"Error creating Fenic session: {e}", file=sys.stderr)
            raise
    return session
...
def main():
    ...
    try:
        # Test that we can create a session and access basic functionality
        session = get_session()
        ...
        if missing_tables:
            import sys
            print(f"Error: Missing required tables: {missing_tables}", file=sys.stderr)
            print("Please run 'python populate_tables.py' to set up the documentation database.", file=sys.stderr)
            print("This will extract and index the Fenic API documentation.", file=sys.stderr)
            sys.exit(1)
