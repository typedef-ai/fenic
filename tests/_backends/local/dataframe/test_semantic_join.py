...
def test_semantic_join_fails_with_bad_request_timeouts(local_session: Session):
    ...
    with pytest.raises(ValidationError, match=f"The `request_timeout` argument can't be greater than the system's max timeout of {MAX_CLIENT_TIMEOUT} seconds."):
        left.semantic.join(right, "Taking {{left_on}} will help me learn {{right_on}}", left_on=col("course_name"), right_on=col("skill"), request_timeout=MAX_CLIENT_TIMEOUT + 1)