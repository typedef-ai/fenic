...
from fenic.api.session.config import OpenAILanguageModelConfig
...
def test_local_session_with_embedding_models_only():
    ...
    session_config = SessionConfig(
        app_name="test_app",
        semantic=SemanticConfig(embedding_models={"oai-small": OpenAIEmbeddingModelConfig(model_name="text-embedding-3-small", rpm=3000, tpm=1_000_000)}),
    )
    session = Session.get_or_create(session_config)
    session.stop()
...
def test_local_session_with_ambiguous_default_lm():
    ...
    with pytest.raises(ConfigurationError):
        SessionConfig(
            ...
            semantic=SemanticConfig(
                language_models={"mini" :OpenAILanguageModelConfig(model_name="gpt-4o-mini", rpm=500, tpm=200_000),
                                 "nano" : OpenAILanguageModelConfig(model_name="gpt-4.1-nano", rpm=500, tpm=200_000)},
            ),
        )
