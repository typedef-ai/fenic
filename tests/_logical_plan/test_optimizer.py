...
def test_semantic_predicate_rewrite_with_other_semantic_exprs(local_session):
    ...
    golden_repr = dedent(
        """
        Filter(predicate=(semantic.extract_cfff2cfc(blurb1)[lit(blurb)] = lit(blurb)))
...
def test_semantic_predicate_rewrite_complex_with_other_semantic_exprs(local_session):
    ...
    golden_repr = dedent(
        """
        Filter(predicate=((semantic.extract_cfff2cfc(blurb1)[lit(blurb)] = semantic.extract_cfff2cfc(blurb2)[lit(blurb)]) OR semantic.predicate_b5897940(blurb2)))
